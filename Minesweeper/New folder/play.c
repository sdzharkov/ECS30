...
void get_play(Board board) {
	...
	while(1){
		...
		if (check_valid(board, enter_row, enter_col)) {
...
void mine_location_generator(Board board) {
	...
	while (i != 0) {
		...
		row = board.row - rand() % board.row - 1; //choose a ranom row
		col = rand() % board.col; //chose a random column 
		if (board.tile[row][col].concealed == '*') { //if there is already a mine there skip to the next one 
			continue;
		}
		else {
			board.tile[row][col].concealed = '*';
			printf("Placing mine at %d, %d\n", board.row - row - 1, col); // tell the user where you are placing the mine 
		} 
		i--;
	}
...
char count_mine(Board board, int row, int col) {
	int count = 48;
	char count_num;
	if (check_valid(board, row-1, col-1)) {
		if (checkmine(board, row-1, col-1)) {
			count++;
		}
	}
	if (check_valid(board, row, col-1)) {
	...
	if (check_valid(board, row-1, col)) {
