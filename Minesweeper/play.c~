...
char count_mine(Board board, int row, int col) {
	...
	int count = 48;
	char count_num;
//have to check for all 8 surrounding squares of where the mine is placedj
//if it is in bounds then you can check if there is a mine there 
		for (i = 0; i < 8; i++){
			switch(i){ //use switch statements for every possible case 
				case 0:
					...
					break; //need break statements for each case so that multiple cases don't execute 
				case 1:
	...
	count_num = count;
	return count_num; //return the number of mines 
}
